...
// export const requestAd = async (AdType:'InterstitialAd' | 'rewardedAd') =>{
//   const InterstitialAd = "ca-app-pub-7273251331335061/7217862460"
//     const rewardedAd = "ca-app-pub-7273251331335061/4107154403"
//     if(AdType === "InterstitialAd"){
//         await AdMobInterstitial.setAdUnitID(InterstitialAd);
//         await AdMobInterstitial.requestAdAsync({ servePersonalizedAds: false});
//         await AdMobInterstitial.showAdAsync();
//     }else if(AdType === "rewardedAd"){
//         await AdMobRewarded.setAdUnitID(rewardedAd);
...
export const check_if_user_owes = (user:UserProfile)=>{
  let balance = user?.balance || 0;
  const owingDate = user.owingDate;
  if(owingDate !== 'NOT_OWING'){
    let isTimeDue = false;
    if((Date.now() - parseFloat(owingDate)) > (1000 * 60 * 60 * 24 * 2)){
      isTimeDue = true;
    }
    if(balance < -99 && isTimeDue){
      balance = (0 - balance);
